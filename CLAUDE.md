...
`RecurringTask.frequency` is a plain `String` with no dedicated schema for structure — it's either one of 4 simple presets (`DAILY`/`WEEKLY`/`BIWEEKLY`/`MONTHLY`) or a custom day-of-week pattern, and `FrequencyUtils` is the single place that understands both. `isSimple(frequency)` picks the branch; `nextExecution(frequency, from)` does `from.plusX(...)` for presets, or — for anything else — `CronExpression.parse(frequency).next(from)` (`org.springframework.scheduling.support.CronExpression`, built into Spring Framework, no extra dependency). A custom frequency is always a 6-field cron string shaped `"0 0 0 * * <days>"` (seconds/minutes/hours pinned to midnight, since `RecurringTaskSchedulerService` only ticks once a day anyway; `<days>` a comma-separated list of 0-6, Sunday=0, matching JS `Date#getDay()`) — built and parsed on the frontend via `recurrenceUtils.ts` (`buildCustomFrequency`/`parseCustomFrequencyDays`/`formatCustomFrequencyLabel`), used by both `SubjectSection.tsx`'s create form and `RecurringTasksModal.tsx`'s inline edit form (`WeekdayPicker.tsx` is the shared day-chip UI). `RecurringTaskRequestDTO.frequency`'s `@Pattern` is deliberately narrower than a generic cron-charset regex — it only accepts the 4 presets or exactly that day-of-week shape, so every stored frequency the API will ever accept can always be parsed back into day checkboxes for editing; don't loosen it to accept arbitrary cron (day-of-month/month fields, letter-based day names, etc.) without also building UI that can display/edit whatever gets added. `RecurringTaskSchedulerService.generateTask()` wraps each recurring task's processing in its own try/catch — one bad/unparseable `frequency` (e.g. stale data from before this validation existed) logs and is skipped rather than aborting generation for every other active recurring task in that run. `RecurringTaskSchedulerService` generates the next `Task` occurrences; `ProgressRecordSchedulerService` computes recurring progress snapshots. Both run via Spring's `@Scheduled` (`@EnableScheduling` is on in `RedCheckApiApplication`).
